...
window.exportPNG = async () => {
  ...
  if (btn) { btn.textContent = "[wait] Capturing..."; btn.disabled = true; }
  try {
    ...
    if (btn) { btn.textContent = "[dl] PNG"; btn.disabled = false; }
  }
};
...
window.saveTeamsData = () => {
  ...
  if (btn) { btn.textContent = "v Saved & Synced!"; setTimeout(()=>{ btn.textContent = "[save] Save Roster & Team Names"; },2000); }
};
...
window.saveInventory = () => {
  ...
  if (btn) { btn.textContent = "v Saved!"; setTimeout(()=>{ btn.textContent = "[save] Save Inventory"; }, 2000); }
};
...
if (isConfigured) {
  ...
  document.getElementById("root").innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;
      ...
      <div style="font-size:3rem">[ball]</div>
